classdef Cells < handle & matlab.mixin.Copyable %create object by reference
	...
	methods(Access= protected)
		function letItRip(thisObj)
			...
			feedfwdGmatrix=thisObj.internalConnObj.feedfwdGmatrix;
			dendriticDelayTemplateMatrix=thisObj.internalConnObj.dendriticDelayTemplateMatrix;
			...
			for step=1:numSteps-1
				%update L1 cells based on last time step
						...
						if(step>1 && v(cellRow,placeIdx,step)>-30 && v(cellRow,placeIdx,step-1) <-30)
							...
							if(numCellsPerPlace*numPlaces>=1)
								...
								for postSynL2CellIdx=1:numCellsL2
								     ...
								     dendriticDelay=dendriticDelayTemplateMatrix(cellRow,placeIdx,postSynL2CellIdx);
					
								      delay=phasePrecessionDelay+dendriticDelay;
									 %4000 ms covers integrated timecourses with delay
									 synEndStep=step+1+round(delay/dt)+round(4000/dt);
									 if(synEndStep>numSteps)
									     synEndStep=numSteps;
									 end
									 synCurrentIdxes=(step+1+round(delay/dt)):synEndStep;
		
								
								     if(step*dt>=startCouplingTime && weight>0)
									gsynL2(postSynL2CellIdx,synCurrentIdxes)=squeeze(gsynL2(postSynL2CellIdx,synCurrentIdxes))...
									    +weight*exp(-dt*(synCurrentIdxes-(step+1+round(delay/dt)))/tausyn).'; %instantaneous conductance jump with single additive exp decay..
									%count=count+1;
								     end
								end
							end
				...
				for cellNumL2=1:numCellsL2	
					...
					if(step>1 && v(cellNumL2,step)>-30 && v(cellNumL2,step-1) <-30)
					...
					hInc=double(dt*(hinf-hSpecific)/tauh);
				        ...
				        %V=V+dV
				        v(cellNumL2,step+1)=vSpecific+vInc;
				        %Gates=Gates+dGates
				        n(cellNumL2,step+1)=nSpecific+nInc;
				        m(cellNumL2,step+1)=mSpecific+mInc;
				        h(cellNumL2,step+1)=hSpecific+hInc;
			         end
